...
type Config struct {
	...
	AssistApiKey     string          `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://assist.org",
	}
	...
	if v := os.Getenv("ASSIST_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ASSIST_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "assist-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":          token,
		"api_key":        c.AssistApiKey,
		"ASSIST_API_KEY": c.AssistApiKey,
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.AssistApiKey = creds.AssistApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AssistApiKey = token
	delete(c.envOverrides, "AssistApiKey")
	c.updateFileConfigField("AssistApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AssistApiKey"] {
			out.AssistApiKey = c.fileConfig.AssistApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AssistApiKey":
		c.fileConfig.AssistApiKey = c.AssistApiKey
	}
}
