...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  assist-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  assist-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"academic-years", "courses", "departments", "institutions", "prefixes", "year-terms"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  assist-pp-cli workflow status

  # Show status as JSON
  assist-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("assist-pp-cli")
			}
