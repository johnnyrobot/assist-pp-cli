...
type rootFlags struct {
	...
	// invocation. Mirrors the ASSIST_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "assist-pp-cli",
		Short: `Public ASSIST articulation and transferability data with no API key.`,
		Long: `Public ASSIST articulation and transferability data with no API key.

Highlights (not in the official API docs):
  • resolve institution   Turn recognizable institution and academic-year names into exact ASSIST identifiers.
  • advisor agreements   Find official agreements from institution names, catalog year, and agreement types in one call.
	• agreements diff   Compare two official agreement payloads as deterministic added, removed, and changed JSON paths.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'assist-pp-cli doctor' to verify anonymous public connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("assist-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'assist-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAgreementsCmd(flags))
	rootCmd.AddCommand(newTransferabilityCmd(flags))
	...
	rootCmd.AddCommand(newNovelAdvisorCmd(flags))
	rootCmd.AddCommand(newNovelResolveCmd(flags))
	rootCmd.AddCommand(newPublicSettingsCmd(flags))
	rootCmd.AddCommand(newAcademicYearsPromotedCmd(flags))
	rootCmd.AddCommand(newInstitutionsPromotedCmd(flags))
