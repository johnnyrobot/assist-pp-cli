...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{1,9}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: assist-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{1,9}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"agreement_type": {
			{Canonical: "Prefix", Values: []string{"prefix agreement"}},
			{Canonical: "Department", Values: []string{"department agreement", "dept agreement"}},
			{Canonical: "Major", Values: []string{"major agreement"}},
			{Canonical: "GeneralEducation", Values: []string{"general education", "ge agreement"}},
		},
		"transferability_area_type": {
			{Canonical: "IGETC", Values: []string{"intersegmental ge"}},
			{Canonical: "CSUAI", Values: []string{"american ideals"}},
			{Canonical: "CSUGE", Values: []string{"csu ge"}},
			{Canonical: "CALGETC", Values: []string{"cal-getc"}},
			{Canonical: "UCTEL", Values: []string{"uc transfer eligibility"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: assist-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: assist-pp-cli: learn init: %v\n", err)
		}
	})
}
