...
// assistScopedAutoRefreshPolicy anchors the deliberate exclusion of scoped
// catalog resources, which require explicit institution/year parameters.
const assistScopedAutoRefreshPolicy = true
...
var readCommandResources = map[string][]string{
	"assist-pp-cli academic-years": {"academic-years"},
	"assist-pp-cli institutions":   {"institutions"},
	"assist-pp-cli year-terms":     {"year-terms"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "ASSIST_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale assist-pp-cli cache (refresh failed: %v)\n", err)
