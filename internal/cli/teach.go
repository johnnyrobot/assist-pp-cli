...
// assist-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "ASSIST_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("assist-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  assist-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  assist-pp-cli recall "<question>" --agent
  assist-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  assist-pp-cli learnings list --agent
  assist-pp-cli learnings list --query "<substring>"
  assist-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  assist-pp-cli learnings forget "<question>" --resource <id>
  assist-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  assist-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  assist-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
