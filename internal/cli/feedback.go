...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ASSIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ASSIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "assist-pp-cli/feedback")
...
When ` + "`ASSIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ASSIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  assist-pp-cli feedback list
  assist-pp-cli feedback list --limit 5
  assist-pp-cli feedback list --json`,
