...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for ASSIST",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  assist-pp-cli auth setup\n  assist-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://assist.org")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export ASSIST_API_KEY=\"your-token-here\"")
			_, _ = io.WriteString(w, "  printf '%s' \"$ASSIST_API_KEY\" | assist-pp-cli auth set-token --stdin\n")
			if !launch {
				return nil
			}
			launchURL := "https://assist.org"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  assist-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export ASSIST_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  printf '%%s' \"$ASSIST_API_KEY\" | assist-pp-cli auth set-token --stdin\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  printf '%s' \"$ASSIST_API_KEY\" | assist-pp-cli auth set-token --stdin",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 && fromStdin {
				return usageErr(fmt.Errorf("provide the token either as a positional argument or with --stdin, not both"))
			}
			token, readErr := readAuthToken(cmd, args, fromStdin)
			if readErr != nil {
				return readErr
			}
			if !fromStdin {
				fmt.Fprintln(cmd.ErrOrStderr(), "warning: positional tokens may be exposed in shell history; prefer --stdin")
			}
			if token == "" {
	...
	cmd.Flags().BoolVar(&fromStdin, "stdin", false, "Read the token from standard input instead of process arguments")
	return cmd
}

func readAuthToken(cmd *cobra.Command, args []string, fromStdin bool) (string, error) {
	if !fromStdin {
		return args[0], nil
	}
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  assist-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("ASSIST_API_KEY") != "" {
				envStillSet = "ASSIST_API_KEY"
			}
