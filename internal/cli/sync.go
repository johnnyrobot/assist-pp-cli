...
// assistScopedSyncEnabled anchors ASSIST's path-scoped snapshot and reconcile
// extensions across force regeneration.
const assistScopedSyncEnabled = true
...
Resource scoping:
  ...
  # Sync one institution/year catalog snapshot
  assist-pp-cli sync --resources courses,departments,prefixes \
    --resource-param courses.institutionId=110 \
    --resource-param courses.academicYearId=75 \
    --resource-param departments.institutionId=110 \
    --resource-param departments.academicYearId=75 \
    --resource-param prefixes.institutionId=110 \
    --resource-param prefixes.academicYearId=75

  # Full resync (ignore previous checkpoint)
  assist-pp-cli sync --full

  # Parallel sync with 8 workers
  assist-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  assist-pp-cli sync --latest-only`,
...
func syncResource(ctx context.Context, c interface {
	...
	path, assistScope := resolveAssistSyncPath(resource, path, userParams)
	...
	// ASSIST catalog resources are stored as institution/year snapshots. Their
	// composite storage IDs intentionally differ from the bare API IDs, so the
	// generic flat reconciler must not compare or prune them across partitions.
	flatReconcilable := resourceReconcileMode(resource) == "flat" && len(assistScope) == 0
	assistScoped := assistScopedSyncResources[resource] && len(assistScope) != 0
	...
	for {
		...
		items = annotateAssistSyncItems(resource, items, assistScope)
		...
		if flatReconcilable || assistScoped {
	...
	// ASSIST catalogs are complete institution/year snapshots. Record every
	// proven-complete snapshot independently so offline comparisons can reject
	// missing evidence, and on --full reconcile only within this exact scope.
	if assistScoped && outcome.complete {
		scopeValue := assistScope["institutionId"] + ":" + assistScope["academicYearId"]
		if prune {
			deleted, rerr := db.ReconcilePartition(resource, "$._assistScope", scopeValue, seenIDs, resource, nil)
			if rerr != nil {
				fmt.Fprintf(syncEvents, `{"event":"reconcile_error","resource":"%s","scope":"%s","error":%q}`+"\n", resource, scopeValue, rerr.Error())
			} else {
				fmt.Fprintf(syncEvents, `{"event":"reconcile","resource":"%s","scope":"%s","deleted":%d}`+"\n", resource, scopeValue, deleted)
			}
		}
		if err := db.SaveAssistCatalogSnapshot(resource, assistScope["institutionId"], assistScope["academicYearId"], totalCount); err != nil {
			return syncResult{Resource: resource, Count: totalCount, Err: fmt.Errorf("saving ASSIST snapshot state: %w", err), Duration: time.Since(started)}
		}
	}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "academic-years":
		return db.UpsertAcademicYears(data)
	case "agreements":
		return db.UpsertAgreements(data)
	case "courses":
		return db.UpsertCourses(data)
	case "departments":
		return db.UpsertDepartments(data)
	case "institutions":
		return db.UpsertInstitutions(data)
	case "prefixes":
		return db.UpsertPrefixes(data)
	case "transferability":
		return db.UpsertTransferability(data)
	case "year-terms":
		return db.UpsertYearTerms(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"academic-years": "/AcademicYears/api",
		"courses":        "/Courses/api/Institution/{institutionId}/Year/{academicYearId}",
		"departments":    "/Departments/api/Institution/{institutionId}/Year/{academicYearId}",
		"institutions":   "/Institutions/api",
		"prefixes":       "/Prefixes/api/Institution/{institutionId}/Year/{academicYearId}",
		"year-terms":     "/YearTerms/api",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "agreements\x00/articulation/api/Agreements/Department/from/{sendingInstitutionParentId}/to/{receivingInstitutionParentId}/{courseType}/course/{courseIdentifierParentId}/for/{academicYearId}":
		return []string{"result"}
	case "agreements\x00/articulation/api/Agreements":
		return []string{"result"}
	case "agreements\x00/articulation/api/Agreements/Published/from/{institutionId}":
		return []string{"result"}
	case "agreements\x00/articulation/api/Agreements/Published/for/{receivingInstitutionId}/to/{sendingInstitutionId}/in/{academicYearId}":
		return []string{"result"}
	case "agreements\x00/articulation/api/Agreements/Major/from/{sendingInstitutionParentId}/to/{receivingInstitutionParentId}/{courseType}/course/{courseIdentifierParentId}/for/{academicYearId}":
		return []string{"result"}
	}
	return nil
}
