...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the public ASSIST session was rejected; no API key is used."+
			"\n      Run 'assist-pp-cli doctor' to check the anonymous XSRF handshake."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: this anonymous public endpoint unexpectedly requested authentication; run 'assist-pp-cli doctor'.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: the anonymous public endpoint denied the request; run 'assist-pp-cli doctor' and retry later.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("anonymous ASSIST session expired or returned HTML instead of JSON; rerun the command to bootstrap a fresh public session"))
	}
	if len(trimmed) == 0 {
