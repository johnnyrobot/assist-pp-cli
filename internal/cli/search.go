...
In local mode: searches locally synced data only.`,
  ...
  assist-pp-cli search "error timeout"

  # Force local search only
  assist-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  assist-pp-cli search "status" --type academic-years --data-source local
  # JSON output for piping
  assist-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /Departments/api
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/Departments/api", map[string]any{
					"q": query,
				})
				if getErr == nil {
			...
			case "transferability":
				results, err = db.SearchTransferability(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchTransferability(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search transferability failed: %w", searchErr)
					}
