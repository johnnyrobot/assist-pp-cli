...
type Client struct {
	...
	publicSessionMu        sync.Mutex
	publicSessionReady     bool
}

const (
	defaultMaxResponseBytes       int64 = 32 << 20
	defaultMaxBinaryResponseBytes int64 = 64 << 20
	publicAssistUserAgent               = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36"
)
...
func (e *ResponseTooLargeError) Error() string {
	return fmt.Sprintf("response exceeds configured limit of %d bytes", e.Limit)
}
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	c := &Client{
		...
		MaxResponseBytes:       responseLimitFromEnv("ASSIST_MAX_RESPONSE_BYTES", defaultMaxResponseBytes),
		MaxBinaryResponseBytes: responseLimitFromEnv("ASSIST_MAX_BINARY_RESPONSE_BYTES", defaultMaxBinaryResponseBytes),
	}
	// The public ASSIST site is HTTPS-only. Keep redirects on the same
	// origin and refuse a downgrade so its anonymous session cookies never
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if strings.EqualFold(via[0].URL.Scheme, "https") && strings.EqualFold(req.URL.Scheme, "http") {
...
func (c *Client) usesPublicAssistSession() bool {
	u, err := url.Parse(c.BaseURL)
	if err != nil || !strings.EqualFold(u.Scheme, "https") {
		return false
	}
	host := strings.ToLower(u.Hostname())
	return host == "assist.org" || host == "www.assist.org"
}

// ensurePublicSession performs the same anonymous bootstrap as assist.org's
// browser application. The landing page issues a public X-XSRF-TOKEN cookie;
// API requests must echo that value in the identically named header. No user
// credential or persistent secret is involved.
func (c *Client) ensurePublicSession(ctx context.Context) error {
	if !c.usesPublicAssistSession() {
		return nil
	}
	c.publicSessionMu.Lock()
	defer c.publicSessionMu.Unlock()
	if c.publicSessionReady {
		return nil
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, c.BaseURL+"/", nil)
	if err != nil {
		return fmt.Errorf("creating ASSIST public-session request: %w", err)
	}
	req.Header.Set("Accept", "text/html,application/xhtml+xml")
	req.Header.Set("User-Agent", publicAssistUserAgent)
	resp, err := c.HTTPClient.Do(req)
	if err != nil {
		return fmt.Errorf("starting anonymous ASSIST public session: %w", err)
	}
	_, readErr := io.Copy(io.Discard, io.LimitReader(resp.Body, 2<<20))
	_ = resp.Body.Close()
	if readErr != nil {
		return fmt.Errorf("reading ASSIST public-session response: %w", readErr)
	}
	if resp.StatusCode >= 400 {
		return fmt.Errorf("starting anonymous ASSIST public session returned HTTP %d", resp.StatusCode)
	}
	base, _ := url.Parse(c.BaseURL + "/")
	for _, cookie := range c.HTTPClient.Jar.Cookies(base) {
		if cookie.Name == "X-XSRF-TOKEN" && cookie.Value != "" {
			c.publicSessionReady = true
			return nil
		}
	}
	return errors.New("ASSIST public session did not issue X-XSRF-TOKEN")
}

func (c *Client) addPublicSessionHeader(req *http.Request) {
	if !c.usesPublicAssistSession() || c.HTTPClient == nil || c.HTTPClient.Jar == nil {
		return
	}
...
func responseLimitFromEnv(name string, fallback int64) int64 {
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// Public assist.org endpoints use an anonymous anti-forgery session,
	// never an API credential. Dry-run deliberately does not bootstrap it.
	authHeader := ""
	...
	if err := c.ensurePublicSession(ctx); err != nil {
		return nil, 0, err
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		c.addPublicSessionHeader(req)
		if c.usesPublicAssistSession() && req.Header.Get("Content-Type") == "" {
			req.Header.Set("Content-Type", "application/json")
		...
		if req.Header.Get("User-Agent") == "" {
			if c.usesPublicAssistSession() {
				...
				req.Header.Set("User-Agent", "assist-pp-cli/0.1.0")
			}
		}
		...
		if binaryResponse || isBinaryResponseContentType(resp.Header.Get("Content-Type")) {
			limit = c.MaxBinaryResponseBytes
		}
		respBody, err := readResponseBodyLimited(resp, limit)
		_ = resp.Body.Close()
		if err != nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export ASSIST_API_KEY=<your-token> or pipe it to assist-pp-cli auth set-token --stdin")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.AssistApiKey)
	}
