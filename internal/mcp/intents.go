...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("audit_two_published_agreements",
			mcplib.WithDescription("Produce stable JSON paths for additions, removals, and changed values."),
		...
		handleAuditTwoPublishedAgreements,
	)
}

// handleAuditTwoPublishedAgreements runs the audit_two_published_agreements recipe intent: Produce stable JSON paths for additions, removals, and changed values.
func handleAuditTwoPublishedAgreements(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
