...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "assist-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "assist-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("assist", "test")
	RegisterTools(s)
...
func TestMCPArrayBodyEndpointPostsExactArray(t *testing.T) {
	resetMCPPathEnv(t)
	var gotBody string
	...
	t.Setenv("ASSIST_API_KEY", "mcp-test-key")
	handler := makeAPIHandler("POST", "/Courses/api/Institution/{institutionId}/Year/{academicYearId}", true, false, nil, mcpPageConfig{}, []mcpParamBinding{
		...
		{PublicName: "body_json", WireName: "body_json", Location: "body_json", BodyIsArray: true},
	}, []string{"institutionId", "academicYearId"})
	result, err := handler(context.Background(), mcplib.CallToolRequest{Params: mcplib.CallToolParams{Arguments: map[string]any{
		"institution-id": 113.0, "academic-year-id": 75.0, "body_json": `[340948,340950]`,
	}}})
	if err != nil || result == nil || result.IsError {
		t.Fatalf("handler result=%#v err=%v", result, err)
	}
	if gotBody != `[340948,340950]` {
