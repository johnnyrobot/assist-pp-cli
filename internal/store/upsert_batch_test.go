...
// TestUpsertBatch_PopulatesAcademicYearsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed academic_years table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesAcademicYearsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("academic-years", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "academic-years").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "academic_years")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count academic_years: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("academic_years count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
// TestUpsertBatch_PopulatesCoursesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed courses table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesCoursesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("courses", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "courses").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "courses")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count courses: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("courses count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
// TestUpsertBatch_DepartmentsSearchableViaGenericFTS verifies that a
...
// non-FTS5 `--type departments` fallback), so populating it is the
...
func TestUpsertBatch_DepartmentsSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// field is matchable regardless of which columns departments declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("departments", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "departments")
	}
}
...
// TestUpsertBatch_PopulatesPrefixesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed prefixes table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesPrefixesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("prefixes", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "prefixes").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "prefixes")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count prefixes: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("prefixes count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_SetsPrefixesParentID verifies official stable prefix parent
// identifiers populate the typed prefix_parent_id column.
func TestUpsertBatch_SetsPrefixesParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": 101, "prefixParentId": 44, "institutionId": 113, "code":"CHEM"}`),
		json.RawMessage(`{"id": 102, "prefixParentId": 44, "institutionId": 113, "code":"CHEM"}`),
		json.RawMessage(`{"id": 103, "prefixParentId": 45, "institutionId": 113, "code":"MATH"}`),
	}
	if _, _, err := s.UpsertBatch("prefixes", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE prefix_parent_id = ?`, "prefixes")
	if err := db.QueryRow(parentQuery, 44).Scan(&matchedA); err != nil {
		t.Fatalf("count by prefix_parent_id: %v", err)
	}
	if matchedA != 2 {
		t.Fatalf("prefix_parent_id=44 count = %d, want 2", matchedA)
	}
}

// TestUpsertBatch_PrefixesSearchableViaGenericFTS verifies that a
// dependent-resource row stored via UpsertBatch — the exact path dependent
// sync uses (upsertResourceBatch -> UpsertBatch) — is reachable through the
// generic resources_fts index via Store.Search. That index is the substrate
// both `search` surfaces query (the empty-type search directly, and the
// non-FTS5 `--type prefixes` fallback), so populating it is the
// precondition issue #2629 reported missing — dependent resources stored in
// the typed table but absent from any FTS index. This asserts the store layer
// where the claimed defect lived; it does not exercise the CLI search wiring.
func TestUpsertBatch_PrefixesSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// resources_fts indexes the full JSON blob, so a distinctive token in any
	// field is matchable regardless of which columns prefixes declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("prefixes", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	// Assert the probe row actually landed before searching for it: a PK
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "prefixes")
	}
}

// TestUpsertBatch_PopulatesTransferabilityTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed transferability table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesTransferabilityTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("transferability", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "transferability").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "transferability")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count transferability: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("transferability count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

func TestSearchTransferabilityQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("transferability", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchTransferability(query, 10)
		if err != nil {
			t.Fatalf("SearchTransferability(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchTransferability(%q) returned no results", query)
		}
	}
}

// TestUpsertBatch_PopulatesYearTermsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed year_terms table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesYearTermsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("year-terms", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "year-terms").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "year_terms")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count year_terms: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("year_terms count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
